            ...
            for idx in range(len(kv_cache_tensor.shared_by)):
                ...
                if "linear_attn" in layer_name and layer_name not in kv_cache_raw_tensors.keys(
                    ...
                    if not self.model_config.use_mla:
                        ...
                        #                        dsa_k_cache_factor)
                        if self.use_sparse_c8_inderex:
                            dsa_k_scale_cache_size = int((kv_cache_tensor.size // dsa_k_cache_factor) // 129)
                        else:
                            dsa_k_scale_cache_size = 0
                        dsa_k_cache_size = int((
                            kv_cache_tensor.size // dsa_k_cache_factor) - dsa_k_scale_cache_size)
                    else:
                    ...
                    for layer_name_inner in kv_cache_tensor.shared_by:
                        ...
                        if ("attn" in layer_name_inner
                            ...
                            if self.use_sparse and self.use_sparse_c8_inderex:
                                kv_cache_raw_tensors[layer_name_inner] = (k_tensor, v_tensor, dsa_k_cache_tensor, dsa_k_scale_cache_tensor)
                            elif self.use_sparse:
                                kv_cache_raw_tensors[layer_name_inner] = (k_tensor, v_tensor, dsa_k_cache_tensor)
                            else:
                ...
                if isinstance(kv_cache_spec, AttentionSpec):
                    ...
                    if self.use_sparse and raw_dsa_k_tensor is not None:
                        ...
                        if self.use_sparse_c8_inderex:
                            ...
                            c8_indexer_dtype = torch.int8
                        dsa_k_cache_shape = (num_blocks,
                                             kv_cache_spec.block_size, 1, 128)
                        # dsa_k_cache_size = (
                        #     num_blocks
                        # ) * kv_cache_spec.block_size * 128 * dtype.itemsize
                        dsa_k_cache_size = num_blocks * kv_cache_spec.block_size * 128 * c8_indexer_dtype.itemsize
                        dsa_k_cache = raw_dsa_k_tensor[:dsa_k_cache_size].view(
                            c8_indexer_dtype).view(dsa_k_cache_shape)
                        if self.use_sparse_c8_inderex and raw_dsa_k_scale_tensor is not None:
                            c8_scale_indexer_dtype = torch.float8_e4m3fn \
                                if current_soc_type == AscendDeviceType.A5 else torch.float16
                            dsa_k_scale_cache_shape = (num_blocks,
                                                      kv_cache_spec.block_size, 1)
                            dsa_k_scale_cache_size = num_blocks * kv_cache_spec.block_size * 1 * c8_scale_indexer_dtype.itemsize
                            dsa_k_scale_cache = raw_dsa_k_scale_tensor[:dsa_k_scale_cache_size].view(
                                c8_scale_indexer_dtype).view(dsa_k_scale_cache_shape)
